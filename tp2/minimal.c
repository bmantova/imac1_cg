...
static const int screenScaleX = 8;
static const int screenScaleY = 6;
...
int main(int argc, char** argv)
{
	...
	while(loop)
	{
		...
		else
		{
			...
			drawCircle(0);			/* Cercle orange en 1;2 */
			glTranslatef(-1.0,-2.0,0.0);
			
			glRotatef(45,0.0,0.0,1.0);
			glTranslatef(2.0,0.0,0.0);
			glColor3ub(255,0,0);
			drawSquare(0);			/* Carré rouge à 45° en Z en 2;0 */
			glTranslatef(-2.0,0.0,0.0);
			glRotatef(-45,0.0,0.0,1.0);

			glTranslatef(2.0,0.0,0.0);
			glRotatef(45,0.0,0.0,1.0);
			glColor3ub(128,0,180);
			drawSquare(1);			/* Carré violet en 2;0 à 45° en Z */
			glRotatef(-45,0.0,0.0,1.0);
			glTranslatef(-2.0,0.0,0.0);

			if(onRightClick == 1)
			{
				angleRotYellowSquare = sqrt(pow(cursX - clickX,2) + pow(cursY - clickY,2));
			}
			glTranslatef(getX(clickX),getY(clickY),0.0);
			glRotatef(angleRotYellowSquare,0.0,0.0,1.0);
			glColor3ub(255,255,0);
			drawSquare(1);		
			glRotatef(-angleRotYellowSquare,0.0,0.0,1.0);	/* Carré jaune en clickX,clickY qui tourne */
			glTranslatef(-getX(clickX),-getY(clickY),0.0);

			rdBlueCircleX = randint(-screenScaleX,screenScaleX)/2;
			rdBlueCircleY = randint(-screenScaleY,screenScaleY)/2;

			glTranslatef(rdBlueCircleX,rdBlueCircleY,0.0);
			glColor3ub(0,0,255);
			drawCircle(1);			/* Cercle bleu random */
			glTranslatef(-rdBlueCircleX,-rdBlueCircleY,0.0);
		}
